...
export class CinemaComponent implements OnInit {
  ...
  constructor( private cinemasService: CinemaService) { }

  ngOnInit() {
    this.getVilles();
  }
  getVilles(){
    this.cinemasService.getVilles().subscribe(res => {
      this.villes = res;
    });
  }
  ...
  getImage(salle){
    return this.cinemasService.imageUrl + salle.projections._embedded.projections[0].film.id;
  }
